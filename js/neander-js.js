...
function fmt(n) {
  if (n < 10) return "\240\240" + n;
  else if (n < 100) return "\240" + n;
  else return n;
}
...
function setmem(qpos, data, loop) {
  ...
  for (pos = qpos; pos <= 255; pos++) {
    ...
    if (pos == 0 || type[pos - 1] < 1) {
      ...
      // We have an address.
      ltype = -1;
      cods.options[pos].text =
        fmt(pos) + ".  " + fmt(data) + "\240\240\240\240\240[" + data + "]";
    }

    dats.options[pos].text =
      fmt(pos) + ". " + fmt(data) + "  (" + signed(data) + ")";

    if (ltype != type[pos]) type[pos] = ltype;
    else break;
...
function updatemem(form) {
  ...
  //adicionei aqui para melhorar a UX
  form.val.focus();
  form.val.select();
}
...
function stepResult() {
  ...
  if (pc() == 255) cont = false;
  else setpc(pc() + 1);

  if (opc == 15) return false;
  if (op[opc]) return eval("op_" + op[opc] + "(" + arg + ")") && cont;
  return true;
}
...
function bringtab(name) {
  ...
  for (i = 0; i < divs.length; i++) {
    if (divs.item(i).id == "div_" + name) {
      ...
      document.getElementById("link_" + divs.item(i).id.substr(4)).className =
        "inactive";
    }
  }
}
...
function carregarHexmem(txt) {
  if (txt.substr(0, 8) != header) {
    if (!confirm("Cabeçalho incorreto. Prosseguir?")) return 1;
  }
  memp = 0;
  txtp = 8;
...
function impexp_do(operation, form) {
  ...
  eval(
    "impexp_" +
      form.formatname.value +
      "(form, document.getElementById('impexp_optspane'), operation)"
  );
...
function _itemProjetoModelo(id, nome) {
  let modelo = $.parseHTML(
    '<li class="list-group-item">    <div>        <span class="id">14</span>        <span class="nome">            projeto x        </span>    </div>    <div class="btn-group" role="group" aria-label="...">        <button type="button" class="btn btn-primary btn-abrir-projeto">Abrir</button>        <button type="button" class="btn btn-danger btn-excluir-projeto">Excluir</button>    </div></li>'
  )[0];
  ...
  listarItems()
    ...
    .then(ev => {
      $(".btn-excluir-projeto").on("click", function(event) {
        let id = event.target.parentNode.parentNode.querySelector(".id")
          .innerHTML;
        // console.log(id);
